#!/usr/bin/env python3
"""Plot results from C++ demo_lpc2hpc."""
...
# Find the CSV file
csv_path = sys.argv[1] if len(sys.argv) > 1 else "demo_lpc2hpc_cpp.csv"
if not os.path.exists(csv_path):
    # Try build directory
    alt = os.path.join(os.path.dirname(__file__), "..", "build", "demo_lpc2hpc_cpp.csv")
    if os.path.exists(alt):
        csv_path = alt
    else:
        print(f"Error: {csv_path} not found. Run demo_lpc2hpc first.")
        sys.exit(1)
...
ax2.set_title('HPC Control Input (m=1)', fontsize=14)
ax2.set_xlim([0, t[-1]])
ax2.grid(True)

fig.suptitle('demo\_lpc2hpc (C++): Homogeneous Proportional Control', fontsize=15, fontweight='bold')
plt.tight_layout()

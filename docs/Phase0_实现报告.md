...
### 9.5 螺旋轨迹问题与 Lissajous 替代

螺旋轨迹（r=2m, ω=0.2-0.5 rad/s）在三个控制器上均出现 6-9 秒后 Z 轴急剧下坠的发散现象。根因是级联控制结构中姿态跟踪的相位滞后，在大误差时形成正反馈循环（详见 Bug #9）。即使提高姿态增益 10 倍（kR=4→40）仍无法根除。

替换为 Lissajous 曲线（往复运动，无需连续转向）后完美解决：

```
轨迹: x_d=2·sin(0.3t), y_d=1.5·cos(0.4t), z_d=-2+0.5·sin(0.2t)
```
...
添加轨迹加速度前馈 `acc_d` 后，位置反馈仅需修正扰动和建模误差，RMS 改善 100-8000 倍（详见 Bug #10）。

### 9.7 推力倾角限制（参考 Elastic-Tracker）

添加 60° 推力倾角限制：保持力的水平方向，缩放水平分量确保 `tilt ≤ max_tilt`。防止大误差时机体过度倾斜→垂直推力不足→Z 轴下坠的发散循环。两个控制器均已支持，`max_tilt` 可配置。

### 9.8 已知问题

1. **螺旋/圆周轨迹**仍是级联控制器的已知难题——论文中建议使用 Lissajous 或其他不需连续转向的轨迹
2. **姿态 HPC 增益 K1=200 的 P 矩阵不平衡** — K₁⁻¹=0.005 与 I 相差 200 倍，可能影响齐次范数的数值精度
3. **hnorm 数值溢出** — 大状态向量时 `expm(-Gd*s)` 可能溢出，已添加 try/except 保护

---

## 10. Elastic-Tracker 对比

Elastic-Tracker（ZJU Fast Lab, ICRA 2022）使用 Lee et al. (2010) 的 SE(3) 几何 PD 控制器，与我们的 `LeeGeometricPD` 基线同源。关键差异：

| 维度 | Elastic-Tracker | 我们的 HPC |
|------|:--------------:|:---------:|
| 位置回路 | 线性 PD + IMU加速度补偿 | 齐次控制器（||x||^(1+μ) 非线性缩放） |
| 姿态误差 | eR = ½(R_d'R - R'R_d)^∨ | θ_e = Log(R·R_d') |
| 姿态控制 | M = -kR·eR - kOm·ω + ω×Jω | u_hom = ||ξ||^(1+μ)·K·expm(-ln||ξ||·Gd)·ξ |
| 收敛性质 | 指数收敛 | μ<0: 有限时间精确收敛 |
| 倾角限制 | 45° 硬限制 | 60° 可配置 |
| 扰动处理 | IMU 加速度反馈（ka 自适应） | ISS 理论框架 |
| 执行层 | 到电机 RPM（含混合矩阵+电机延迟） | 到推力+力矩 |

**值得借鉴的设计**：
1. **IMU 加速度反馈** `m·ka·(a_des - a_imu)`：用加速度计直接测量外力扰动，误差大时自动关闭（|error|>3 → ka=0）。建议在 Phase 3a 输出反馈阶段集成。
2. **倾角限制**：已实现。
3. **电机级建模**：气动阻力 + 一阶电机延迟，比我们的抽象推力+力矩更接近物理实际。

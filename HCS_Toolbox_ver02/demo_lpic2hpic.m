...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% HPC/HFxTC design by upgrading a linear feedback 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


Klin=[2 -35 1.5 -3]; % the linear feedback gain (provided by manufacturer)


Klin_int=[0.5 -26.66   1.26  -2.73]; % the integral gain should be provided by manufacturer

[K0 G0 P Ki mu1 mu2]=lpic2hpic(A,B,Klin, Klin_int); % upgrade linear PI control to HPIC


%selection of the homogeneity degree

mu=0.16; % for HCP with negative homogeneity degree
%Gd=eye(4)+mu2*G0; mu=mu2; % for HCP with positive homogeneity degree
...
alpha=0.001; beta=1000;   %for HPC with negative degree (upgrade close to zero)
%alpha=1;beta=100;   %for HPC with positive degree (upgrade close to Inf)                    
%alpha=0.01;beta=2; %for HFxTC (global upgrade)                    
...
while t<Tmax
    xm=x+2*noise*(rand(4,1)-0.5);
     
    u=Klin*xm;ui=Ki*xm;                                                    %linear PI control (for comparion) 

    [u ui]=e_hpic(xm,K0,K,Ki,Gd,mu,@(x)hnorm(x,Gd,P),alpha,beta);          %HPIC
    
   % [u ui]=e_fhpic(xm,K0,K,Ki,G0,P,mu1,mu2,alpha,beta);                   %FHPIC (for comparison)
   
    u=u+v;v=v+h*ui; 

    %u=Klin*xm;                                                             %linear control (for comparion)

    x=x+h*A*x+h*B*(u+p); %explicit Euler method 
    t=t+h;
    tl=[tl t];
    xl=[xl x];
    ul=[ul u];
